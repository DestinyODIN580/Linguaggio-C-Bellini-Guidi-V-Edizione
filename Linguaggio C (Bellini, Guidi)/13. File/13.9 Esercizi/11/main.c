/* 137.
    Modificare il programma dell’Esercizio 6 in modo tale che il file rubrica contenga, oltre al nome e al numero di
    telefono, anche il cognome e l’indirizzo (via, CAP, città e stato) dei conoscenti memorizzati.
*/
...
void new (int *lastIndex, char mat[][LEN])
{
    ...
    *(lastIndex) += 1;
    ...
    /* importazione in mat di address */
    for (i = 0; i < strlen (address); i++)
        mat[*lastIndex][i + j] = address[i];
    mat[*lastIndex][i + j] = ' ';
    j += i + 1;
    ...
    /* importazione in mat di cap */
    for (i = 0; i < strlen (cap); i++)
        mat[*lastIndex][i + j] = cap[i];
    mat[*lastIndex][i + j] = ' ';
    j += i + 1;
    ...
    /* importazione in mat di city */
    for (i = 0; i < strlen (city); i++)
        mat[*lastIndex][i + j] = city[i];
    mat[*lastIndex][i + j] = ' ';
    j += i + 1;

    /* costruzione di coutry */
    printf ("Inserire il paese > ");
    x = 0;
    while ((c = getchar ()) != '\n')
        country[x++] = c;
    country[x] = '\0';

    /* importazione in mat di country */
    for (i = 0; i < strlen (country); i++)
        mat[*lastIndex][i + j] = country[i];
    mat[*lastIndex][i + j] = '\0';
